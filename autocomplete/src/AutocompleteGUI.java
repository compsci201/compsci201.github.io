...
public class AutocompleteGUI extends JFrame {
  ...
  public AutocompleteGUI(String fileName, int k, String className) {
    ...
    layout.setVerticalGroup(layout.createSequentialGroup().addGroup(
        layout.createParallelGroup(GroupLayout.Alignment.BASELINE).addComponent(textLabel)
  ...
  private class AutocompletePanel extends JPanel {
	...
	try {
      ...
      searchText.setMaximumSize(new Dimension(searchText.getMaximumSize().width, searchText
      ...
      suggestions.setMaximumSize(new Dimension(searchText.getMaximumSize().width, suggestions
          .getPreferredSize().height));
      suggestions.setPrototypeCellValue(suggListLen); // set to make equal to the width of the
                                                      // textfield
      ...
      this.setMaximumSize(new Dimension(searchText.getMaximumSize().width,
    ...
    /**
     * Makes a call to the implementation of Autocomplete to get suggestions for the currently
     * entered text.
     * 
      ...
      if (text.equals("")) {
        ...
        if (!resultQ.isEmpty()) {
          ...
          for (int i = 0; i < results.length; i++) {
            ...
            results[i] =
                "<html>" + results[i].substring(0, textLen) + "<b>" + results[i].substring(textLen)
                    + "</b></html>";
          }
          suggestions.setListData(results);
          suggestions.setVisible(true);
          // suggestions.setSelectedIndex(0); // Pressing enter automatically selects the first one
          // if nothing has been
        } else {
          // No suggestions
          suggestions.setVisible(false);
  ...
  /**
   * Creates a URI from the user-defined string and searches the web with the selected search engine
