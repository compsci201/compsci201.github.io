...
public class HuffViewer {
	...
	private void compress(File original, StatusBar status, HuffPanel panel) {
		...
		try {
			...
			Thread thread = new Thread(() -> {
				...
				updater.start();
				updater.setGenerator(() -> {return in.getBitsRead() / (8.0 * original.length());});
				
				try {
					Processor processor = getProcessor();
					double start = System.currentTimeMillis();
					processor.compress(in, out);
					updater.addContent("Info", getInfo(new double[] {System.currentTimeMillis() - start}, new File[] {original}, new File[] {compressed}));
					updater.stop();
		...
		try {
			...
			Thread thread = new Thread(() -> {
				...
				try {
					...
					updater.addContent("Info", getInfo(new double[] {System.currentTimeMillis() - start}, new File[] {original}, new File[] {decompressed}));
					updater.stop();
	...
	private void compare(File fileA, File fileB, StatusBar status, HuffPanel panel) {
		...
		Thread thread = new Thread(() -> {
			updater.setGenerator(() -> {return inA.getBitsRead() / (8.0 * fileA.length());});
			...
			try {
				...
				while ((bitA = inA.readBits(1)) != -1 & (bitB = inB.readBits(1)) != -1) {
					if (bitA != bitB) {
						...
						inA.close();
						inB.close();
						updater.addContent("Info", getSimpleInfo(System.currentTimeMillis() - start, fileA.length(), fileB.length(), count));
						return;
					}
					count++;
				}
				updater.addContent("Info", getSimpleInfo(System.currentTimeMillis() - start, fileA.length(), fileB.length(), -1));
				updater.stop();
				inA.close();
				inB.close();
				if (bitA == bitB) {
	...
	public enum Status {
		...
		Complete(Color.DARKGREEN, "100%"),
		Working(Color.DARKGRAY, ""),
		Failed(Color.DARKRED, "  0%");
